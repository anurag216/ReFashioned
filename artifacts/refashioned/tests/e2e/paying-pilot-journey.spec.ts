...
test("[pilot:onboarding] creates and renames an organization through customer boundaries", async ({ page }) => {
  const password = process.env.PLAYWRIGHT_E2E_PASSWORD;
  if (!password) throw new Error("PLAYWRIGHT_E2E_PASSWORD is required for the local pilot fixture");
  await page.goto("/");
...
test("[pilot:workspace] Maison Verde paying-pilot browser smoke journey", async ({ page }) => {
  ...
  await expect(page.getByText("CO₂: Not recorded").first()).toBeVisible();
  await expect(page.getByText("Water: Not recorded").first()).toBeVisible();
  ...
  for (const removedPath of ["/settings/billing", "/regulatory-radar", "/carbon-calculator"]) {
