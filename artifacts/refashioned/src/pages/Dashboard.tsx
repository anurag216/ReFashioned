...
export function Dashboard({ onViewMetrics: _onViewMetrics }: { onViewMetrics?: () => void }) {
  ...
  return <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
    ...
    <section aria-labelledby="action-center-title" className="bg-card border rounded-xl p-5 space-y-4">
      ...
      <div className="divide-y">{actions.slice(0,8).map((action,index)=>{const meta=priorityMeta[action.priority];const href=action.product_id?`/products/${action.product_id}`:action.destination;return <div key={`${action.category}-${action.entity_id}-${index}`} className="py-3 flex gap-3 items-start"><span className={`text-[10px] font-bold border rounded px-2 py-1 ${meta.className}`}>{meta.label}</span><div className="min-w-0 flex-1"><p className="font-medium text-sm">{action.title}</p><p className="text-xs text-muted-foreground mt-1">{action.explanation}</p></div><Link href={href} className="text-sm text-primary flex items-center gap-1">Open <ArrowRight className="w-3 h-3"/></Link></div>})}</div>
    </section>
