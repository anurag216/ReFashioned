...
export function TeamJoin() {
  ...
  useEffect(()=>{
    ...
    else void supabase.rpc("get_organization_member_invite_metadata",{p_token:token}).then(({data,error}:{data:Metadata[]|null,error:unknown})=>setMetadata(error||!data?.[0]?{invitation_state:"invalid",organization_name:null,masked_email:null,role:null,expiration:null}:data[0]));
  ...
  async function redeem(){if(!supabase||!token)return;setBusy(true);setError(null);const {error}=await supabase.rpc("redeem_organization_member_invite",{p_token:token});if(error){setError(error.message);setBusy(false);return;}window.history.replaceState({},"","/dashboard");window.location.reload();}
