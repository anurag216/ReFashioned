...
export function DigitalProductPassport({ onBack }: { onBack: () => void }) {
  ...
  const publicationBlocked = !readiness || readiness.blocker_count > 0;
  ...
  return <div className="min-h-screen bg-[#F8FAFC]">
    ...
    <main className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
      ...
      {publicationBlocked && <section role="alert" className="rounded-xl border border-amber-300 bg-amber-50 p-5 text-amber-950"><h2 className="font-semibold">{readiness ? "Resolve readiness blockers before publication" : "Readiness unavailable"}</h2><p className="mt-1 text-sm">{readiness ? "The public passport cannot be published or updated until the product readiness requirements are satisfied." : "Publication is disabled because product readiness could not be verified. Reload or try again before publishing."}</p><ul className="mt-3 list-disc space-y-1 pl-5 text-sm">{(readiness?.blockers ?? []).map(blocker => <li key={blocker}>{blocker}</li>)}</ul></section>}
