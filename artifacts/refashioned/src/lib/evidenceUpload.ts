...
export type EvidenceUploadStep = "authorizing" | "uploading" | "finalizing";
...
export async function uploadEvidenceDocument(
  ...
  const intent = data?.[0];
  if (intentError || !intent) return { error: `Evidence upload authorization failed: ${intentError?.message ?? "try again"}` };
  ...
  if (uploadError) {
    ...
    return { error: `Evidence upload failed: ${uploadError.message}. Retry to reuse this stage.` };
  }
  ...
  if (finalizeError) return { error: `Evidence finalization failed: ${finalizeError.message}. Retry finalization before starting another upload.` };
  return { error: null };
}
