...
export function TeamAccess(){
 ...
 <section className="bg-card border rounded-lg p-5"><h3 className="font-semibold mb-3">Invitations</h3><div className="space-y-3">{view?.invites.map(i=><div key={i.invite_id} className="flex flex-wrap items-center gap-3 border-b pb-3"><span className="flex-1 min-w-52">{i.email}</span><span className="capitalize">{i.role}</span><span className="text-sm text-muted-foreground">{new Date(i.expires_at).toLocaleDateString()} · {i.state}</span>{i.state==="usable"&&<button onClick={()=>void revokeInvite(i)} className="rounded border px-3 py-2 text-red-700">Revoke invitation</button>}</div>)}</div></section></div>
}
