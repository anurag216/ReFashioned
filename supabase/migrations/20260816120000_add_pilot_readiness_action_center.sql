    ...
    FROM (
      ...
      FROM (
        SELECT p.id product_id,p.name product_name,
          ...
          (SELECT count(*) FROM public.certifications c JOIN public.evidence_uploads e ON e.id=c.evidence_id JOIN public.lifecycle_stages s ON s.id=e.lifecycle_stage_id WHERE s.product_id=p.id AND c.organization_id=v_org AND e.organization_id=v_org AND c.verification_status='verified' AND c.expiry_date BETWEEN current_date AND current_date+30 AND e.status='approved' AND e.scan_status='clean' AND e.content_sha256 IS NOT NULL) expiring_count,
