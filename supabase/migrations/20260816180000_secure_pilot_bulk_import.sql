...
CREATE FUNCTION public.pilot_import_context(p_batch_id uuid DEFAULT NULL)
...
BEGIN
 ...
 ELSE SELECT b.* INTO v FROM public.pilot_import_batches b JOIN public.organization_members m ON m.organization_id=b.organization_id WHERE b.id=p_batch_id AND m.profile_id=auth.uid() AND m.role IN ('admin','manager');
 END IF;
...
BEGIN c:=public.pilot_import_context(NULL);
...
BEGIN b:=public.pilot_import_context(p_batch_id);
...
BEGIN b:=public.pilot_import_context(p_batch_id);
 ...
 FOR r IN SELECT * FROM public.pilot_import_rows WHERE batch_id=b.id ORDER BY row_number LOOP
  ...
  ELSIF b.import_type='suppliers' THEN
   ...
   IF n->>'contact_email' IS NOT NULL AND n->>'contact_email' !~ '^[^[:space:]@]+@[^[:space:]@]+[.][^[:space:]@]+$' THEN e:=e||'"Invalid contact_email"'::jsonb; END IF;
  ...
  ELSIF b.import_type='product_materials' THEN
   ...
   IF v IS NOT NULL AND (SELECT coalesce(sum(CASE WHEN (x.raw_payload->>'composition_percentage') ~ '^[0-9]+([.][0-9]+)?$' THEN (x.raw_payload->>'composition_percentage')::numeric ELSE 0 END),0) FROM public.pilot_import_rows x WHERE x.batch_id=b.id AND lower(btrim(x.raw_payload->>'product_sku'))=lower(n->>'product_sku'))>100 THEN e:=e||to_jsonb('Composition total for SKU '||(n->>'product_sku')||' exceeds 100%'); END IF;
  ELSE
...
BEGIN PERFORM pg_advisory_xact_lock(hashtextextended(p_batch_id::text,0)); b:=public.pilot_import_context(p_batch_id); SELECT * INTO b FROM public.pilot_import_batches WHERE id=b.id FOR UPDATE;
 ...
 END LOOP;
 UPDATE public.pilot_import_batches SET status='completed',committed_at=now() WHERE id=b.id;
