...
BEGIN
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'active admin or manager membership required' USING ERRCODE='42501'; END IF;
 IF btrim(coalesce(p_name,''))='' THEN RAISE EXCEPTION 'product name is required' USING ERRCODE='22023'; END IF;
 SELECT status INTO v_status FROM public.products WHERE id=p_product_id AND organization_id=v_org FOR UPDATE;
 ...
 UPDATE public.products SET name=btrim(p_name),sku=nullif(btrim(p_sku),''),season=nullif(btrim(p_season),'') WHERE id=p_product_id;
END $$;
...
BEGIN
 SELECT m.organization_id INTO v_org FROM public.organization_members m JOIN public.organizations o ON o.id=m.organization_id
 JOIN public.products p ON p.organization_id=m.organization_id AND p.id=p_product_id AND p.status<>'archived'
 WHERE m.profile_id=auth.uid() AND m.role IN ('admin','manager') AND o.lifecycle_status='active';
 IF NOT FOUND THEN RAISE EXCEPTION 'editable product not found' USING ERRCODE='42501'; END IF;
 IF btrim(coalesce(p_material_name,''))='' OR p_composition_percentage<=0 OR p_composition_percentage>100 THEN RAISE EXCEPTION 'invalid material' USING ERRCODE='22023'; END IF;
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE v_org uuid; v_product uuid; v_total numeric;
BEGIN
 SELECT p.organization_id,pm.product_id INTO v_org,v_product FROM public.product_materials pm JOIN public.products p ON p.id=pm.product_id
 JOIN public.organizations o ON o.id=p.organization_id JOIN public.organization_members m ON m.organization_id=p.organization_id
 WHERE pm.id=p_material_id AND p.status<>'archived' AND o.lifecycle_status='active' AND m.profile_id=auth.uid() AND m.role IN ('admin','manager');
 ...
 IF v_total+p_composition_percentage>100 THEN RAISE EXCEPTION 'material composition exceeds 100%%' USING ERRCODE='23514'; END IF;
 UPDATE public.product_materials SET material_name=btrim(p_material_name),composition_percentage=p_composition_percentage,certification_required=coalesce(p_certification_required,false) WHERE id=p_material_id;
...
BEGIN
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'editable material not found' USING ERRCODE='42501'; END IF;
 PERFORM pg_advisory_xact_lock(hashtextextended(v_product::text,0));
 DELETE FROM public.product_materials WHERE id=p_material_id;
