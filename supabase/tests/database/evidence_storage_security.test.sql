...
SELECT ok((SELECT with_check NOT ILIKE '%metadata%' AND with_check NOT ILIKE '%evidence_uploads%' AND with_check ILIKE '%owner_id%' AND with_check ILIKE '%current_actor_can_preflight_evidence_object%' FROM pg_catalog.pg_policies WHERE schemaname='storage' AND tablename='objects' AND policyname='compliance_docs_insert'),'Storage INSERT policy uses owner_id and private preflight decision without metadata or direct evidence access');
SELECT ok((SELECT p.prosecdef AND p.prorettype='boolean'::regtype AND p.proargtypes='25 25'::oidvector AND p.proconfig @> ARRAY['search_path=pg_catalog'] AND NOT EXISTS(SELECT 1 FROM pg_catalog.aclexplode(coalesce(p.proacl,pg_catalog.acldefault('f',p.proowner))) acl WHERE acl.grantee=0 AND acl.privilege_type='EXECUTE') AND NOT has_function_privilege('anon',p.oid,'EXECUTE') AND has_function_privilege('authenticated',p.oid,'EXECUTE') AND NOT has_function_privilege('service_role',p.oid,'EXECUTE') FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='current_actor_can_preflight_evidence_object'),'private preflight helper is a fixed boolean SECURITY DEFINER decision with narrow privileges');
