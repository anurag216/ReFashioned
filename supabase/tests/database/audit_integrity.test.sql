...
RESET ROLE;
CREATE TEMP TABLE audit_member AS SELECT id FROM public.organization_members WHERE profile_id='a0000000-0000-0000-0000-000000000005';
SELECT set_config('request.jwt.claim.sub','a0000000-0000-0000-0000-000000000001',true);
...
SET LOCAL ROLE authenticated;
SELECT lives_ok(format('SELECT public.update_organization_member_role(%L,%L,%L)',(SELECT id FROM audit_member),'viewer','Audit role change'),'admin changes member role through RPC');
SELECT lives_ok(format('SELECT public.update_organization_member_role(%L,%L,%L)',(SELECT id FROM audit_member),'viewer','Audit same-role no-op'),'membership RPC no-op succeeds');
SELECT lives_ok(format('SELECT public.revoke_organization_member_access(%L,%L)',(SELECT id FROM audit_member),'Audit access revocation'),'admin removes member through RPC');
SELECT is((SELECT count(*) FROM public.audit_logs WHERE action='organization_member_added' AND profile_id='a0000000-0000-0000-0000-000000000005' AND entity_name=(SELECT id::text FROM audit_member)),1::bigint,'member addition records exactly one invitee actor and membership ID');
SELECT is((SELECT count(*) FROM public.audit_logs WHERE action='organization_member_role_changed' AND profile_id='a0000000-0000-0000-0000-000000000001' AND entity_name=(SELECT id::text FROM audit_member)),1::bigint,'only the real role change records one admin event');
SELECT is((SELECT count(*) FROM public.audit_logs WHERE action='organization_member_removed' AND profile_id='a0000000-0000-0000-0000-000000000001' AND entity_name=(SELECT id::text FROM audit_member)),1::bigint,'member removal records exactly one admin actor and membership ID');
