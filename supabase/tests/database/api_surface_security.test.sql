...
SELECT is(
  ...
  ARRAY[
    ...
    'stage_pilot_import_rows(p_batch_id uuid, p_rows jsonb)','unpublish_product_passport(p_product_id uuid)','update_organization_profile(p_name text)','update_organization_member_role(p_member_id uuid, p_new_role text, p_reason text)','update_product_material(p_material_id uuid, p_material_name text, p_composition_percentage numeric, p_certification_required boolean)','update_product_metadata(p_product_id uuid, p_name text, p_sku text, p_season text)','update_supplier_contact(p_supplier_contact_id uuid, p_name text, p_email text)',
