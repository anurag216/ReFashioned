...
SELECT is(
  ...
  ARRAY[
    ...
    'create_pilot_import_batch(p_import_type text, p_file_name text)','create_product_material(p_product_id uuid, p_material_name text, p_composition_percentage numeric, p_certification_required boolean)','create_supplier_contact(p_supplier_id uuid, p_name text, p_email text)',
    ...
    'get_organization_action_center()','get_organization_member_invite_metadata(p_token text)','get_organization_product_readiness()','get_organization_sustainability_report()','get_pilot_import_batch(p_batch_id uuid)',
    'get_product_passport_preview(p_product_id uuid)','get_product_passport_publication_state(p_product_id uuid)','get_product_workspace(p_product_id uuid)',
    ...
    'redeem_organization_member_invite(p_token text)','redeem_supplier_invite(p_token text)','remove_product_material(p_material_id uuid)','request_organization_deletion()','request_personal_data_erasure()','review_evidence_upload(p_evidence_id uuid, p_decision text, p_rejection_reason text)',
    ...
    'revoke_supplier_invite(p_invitation_id uuid)','rollback_lifecycle_stage_without_evidence(p_stage_id uuid)','rotate_product_passport_slug(p_product_id uuid)',
    'stage_pilot_import_rows(p_batch_id uuid, p_rows jsonb)','unpublish_product_passport(p_product_id uuid)','update_organization_member_role(p_member_id uuid, p_new_role text, p_reason text)','update_organization_profile(p_name text)','update_product_material(p_material_id uuid, p_material_name text, p_composition_percentage numeric, p_certification_required boolean)','update_product_metadata(p_product_id uuid, p_name text, p_sku text, p_season text)','update_supplier_contact(p_supplier_contact_id uuid, p_name text, p_email text)',
    'validate_pilot_import_batch(p_batch_id uuid)'
...
SELECT ok(has_table_privilege('authenticated','public.product_materials','SELECT'),'authenticated retains scoped product material read');
SELECT ok(NOT has_table_privilege('authenticated','public.product_materials','INSERT'),'authenticated cannot directly insert product materials');
SELECT ok(NOT has_table_privilege('authenticated','public.product_materials','UPDATE'),'authenticated cannot directly update product materials');
SELECT ok(NOT has_table_privilege('authenticated','public.product_materials','DELETE'),'authenticated cannot directly delete product materials');
